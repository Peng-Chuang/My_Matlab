...
% ----------------------------------------------- 文件路径 ---------------------------------------- %
dir_name = 'C:\Users\pengc\Desktop\911#电弧日志\400M';
...
for i=1:length(file_name)
    ...
    % 前一次4个半波，本次1个半波，则上次的第1个半波被覆盖，但上次的后3个半波仍存有值。
    zhwen_bytes_index = [];            
    valid_bytes_index = [];
    ...
    for ii=1:length(del_patch_index)
        for jj=1:59
            if (jj <= 6)
                ...
                data_end1 = ceil(base/20) * 20;
                differ1 = data_end1 - base;
                del_index = del_patch_index(ii) + data_end1 * jj -differ1+1: ...
                                del_patch_index(ii) + data_end1 * jj;
            elseif (jj<=15)
                base = valid_num(ii) * FFT_Len;
                data_end2 = ceil(base/20) * 20;
                differ2 = data_end2 - base; 
                del_index = del_patch_index(ii) + data_end2 * (jj-6) + 6*data_end1 -differ2+1: ...
                                del_patch_index(ii) + data_end2 * (jj-6) + 6*data_end1; 
            elseif (jj<=19)
                base = valid_num(ii) * DISP_Len;
                data_end3 = ceil(base/20) * 20;
                differ3 = data_end3 - base;
                del_index = del_patch_index(ii)+data_end3*(jj-15)+6*data_end1+9*data_end2 -differ3+1: ...
                                del_patch_index(ii)+ data_end3 *(jj-15)+6*data_end1+9*data_end2;
            elseif (jj<=57)
                base = valid_num(ii) * FFT_Len;
                data_end4 = ceil(base/20) * 20;
                differ4 = data_end4 - base;
                del_index = del_patch_index(ii) + data_end4*(jj-19)+6*data_end1+9*data_end2+4*data_end3-differ4+1: ...
                                del_patch_index(ii)+data_end4*(jj-19)+6*data_end1+9*data_end2+4*data_end3;
            elseif (jj<=59)
                base = meter_valid_num(ii) * MSR_V_Len;
                data_end5 = ceil(base/20) * 20;
                differ5 = data_end5 - base;
                del_index = del_patch_index(ii) + data_end5 * (jj-57)+6*data_end1+9*data_end2+4*data_end3+38*data_end4 -differ5+1: ...
                                    del_patch_index(ii) + data_end5 * (jj-57)+6*data_end1+9*data_end2+4*data_end3+38*data_end4;                     
            end
